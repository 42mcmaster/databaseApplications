...
**You'll learn these in future lessons:**

### Aggregate Functions (Lesson 06)
```sql
SELECT COUNT(*) FROM passengers;
SELECT AVG(Age) FROM passengers;
SELECT MAX(Fare), MIN(Fare) FROM passengers;
```

### GROUP BY (Lesson 06)
```sql
SELECT Sex, COUNT(*) FROM passengers GROUP BY Sex;
```

### JOINs (Lesson 08)
```sql
SELECT * FROM passengers 
JOIN tickets ON passengers.TicketID = tickets.TicketID;
```

**For now:** Focus on SELECT, WHERE, ORDER BY, LIMIT!
